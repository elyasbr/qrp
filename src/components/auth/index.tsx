"use client";
import { LockIcon, Phone } from "lucide-react";
...
export default function Auth() {
  ...
  const handleSendCode = () => {
    ...
    setError("");
    console.log("ارسال کد برای:", phone);
    setStep("code");
  };
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.1)] p-8 space-y-8 transition">
        ...
        {step === "code" && (
          <div className="space-y-4">
            ...
            <div className="flex items-center border border-gray-300 rounded-lg focus-within:ring-2 focus-within:ring-[var(--main-color)] transition">
              <LockIcon className="mx-3 text-[var(--main-color)]" size={20} />
              <input
                type="text"
                value={code}
                onChange={(e) => setCode(e.target.value)}
                placeholder="کد ارسال شده را وارد کنید"
                className="w-full p-3 text-center placeholder-gray-400 bg-transparent focus:outline-none tracking-widest"
                maxLength={6}
              />
            </div>
            {error && <p className="text-red-500 text-sm">{error}</p>}
